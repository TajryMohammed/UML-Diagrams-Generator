...
public class EntityRelationAnalyzer {
	...
	private void printRelationshipInfo(ModRelationship relationship) {
		System.out.println(
				relationship.getRelationshipType() + " between " + relationship.getSourceEntity().getEntityName()
						+ " and " + relationship.getTargetEntity().getEntityName());
	}
	...
	public ModRelationship parseRealization(ModEntity source, ModEntity target) {
		return (target instanceof ModClass && source instanceof ModInterface
				...
				&& ((ModClass) target).getImplementedInterfaces().contains(target.getEntityName()))
						? createRelationship(target, source, RelationshipType.IMPLEMENTATION)
						: null;
	}
	...
	public ModRelationship parseAggregation(ModEntity source, ModEntity target) {
		return (target instanceof ModClass && source instanceof ModClass && checkAggregationConditions(target, source))
	...
	public ModRelationship parseDependency(ModEntity source, ModEntity target) {
		if (source == null || source.getEntityMethods() == null) {
			return null;
		}

		return source.getEntityMethods().stream()
				.filter(method -> ClassAnalyzer.isMethodParameter(target.getEntityName(), method)).findFirst()
				.map(method -> createRelationship(target, source, RelationshipType.DEPENDENCY)).orElse(null);
	}
